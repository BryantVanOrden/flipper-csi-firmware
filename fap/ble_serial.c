 ...
 * ami_tool). Bypasses the stock profile so RX callbacks reach the app reliably.
 */
...
static void
    ...
    // Advertise a standard "Flipper <name>" so the CSI apps (which scan for the
    // "Flipper" prefix) find it.
    snprintf(
        config->adv_name,
